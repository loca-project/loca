...
| 含む | まだ無い（タスク） |
|---|---|
| 地図・マーカー閲覧・検索・範囲指定検索、画面下の「フィルター」（ADR 0015）、密集地のクラスタ（ADR 0023）、条件と選んだマーカーの共有 URL（T42）、サイドメニューを開いたときのサイト名（ADR 0026・0027） | 最近のアクティビティ（T80） |
| タグ（映っているもの・雰囲気が必須。ADR 0014）、雰囲気 6 色のマーカー | 撮影リクエストへの回答（T43） |
| 撮影機器 4 段（分類 → メーカー → シリーズ → モデル。ADR 0018）、機器マスタの管理者画面での編集（同期で反映。ADR 0025） | |
...
| 撮影リクエスト（内訳・取り下げで熱量が戻る） | Edge AI 検索（T30）、特集（T46） |
| 通報（1 人 1 件で人数を数える。表示は続く）、新着マーカーの RSS（`feed.xml`。T47） | API キーのリファラー制限・App Check（T32） |
| 管理者モード（統計・ユーザー管理とブラックリスト・投稿動画・撮影リクエスト・機器・定期処理・通報・ログ。書き出しは投稿動画と撮影リクエストで）、日本語・英語（英語の辞書は英語のときだけ読む） | ユーザー管理の検索・欄の大きさ（T74）、管理者への昇格と一般への戻し（T75） |
