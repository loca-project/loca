...
| 消えた動画の毎晩の論理削除、30 日後の物理削除（ADR 0021）、プロフィール（ニックネームは重複不可・変えると投稿者名も追従。ADR 0019）、アカウント削除 | チャンネルのバッジ（T55）、足あと（T45）、投稿日を毎時で取る（T57） |
| Google ログイン、マーカーの投稿・編集・削除（即時反映）、自分の投稿（統計・投稿動画・撮影リクエスト。一括の削除と書き出し） | 古いタブへの読み直しの案内（T56） |
| 撮影リクエスト（内訳・取り下げで熱量が戻る） | いいね（T29）、Edge AI 検索（T30）、特集（T46）、RSS（T47） |
| 通報（1 人 1 件で人数を数える。表示は続く） | |
| 管理者モード（統計・エクスポート・ログ・ユーザー管理とブラックリスト・マーカー管理・通報・定期処理）、日本語・英語 | 初期読み込みの JS の削減（T62） |
