...
| 再生数・投稿日・長さ（毎晩 Actions が YouTube Data API で更新、登録直後の分は毎時。ADR 0017）と、それを使うランキング・フィルタ | |
| 消えた動画の毎晩の論理削除、30 日後の物理削除（ADR 0021）、プロフィール（ニックネームは重複不可・変えると投稿者名も追従。ADR 0019）、アカウント削除、自己申告の YouTube チャンネルと本人のチャンネルのバッジ（毎晩の照合。ADR 0029） | 足あと（T45） |
...
| 撮影リクエスト（内訳・取り下げで熱量が戻る）、動画で応える・依頼者が受け取ると熱量が戻り動画に炎が貯まる（ADR 0028）、投稿者の公開プロフィール（投稿者名から開く。投稿数・受け取ったいいね・炎。T82） | Edge AI 検索（T30）、特集（T46） |
| 通報（1 人 1 件で人数を数える。表示は続く）、新着マーカーの RSS（`feed.xml`。T47） | API キーのリファラー制限・App Check（T32） |
| 管理者モード（統計・ユーザー管理とブラックリスト（区画ごとの検索。T74）・投稿動画・撮影リクエスト・機器・定期処理・通報・ログ。書き出しは投稿動画と撮影リクエストで）、日本語・英語（英語の辞書は英語のときだけ読む） | 管理者への昇格と一般への戻し（T75） |
