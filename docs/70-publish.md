...
索引（`firestore.indexes.json`）も同じ `deploy:rules` で反映する（T82）。投稿者ごとのいいね・炎の合計（`where ownerUid ==` と `sum`）は、
本番では複合索引が無いと「The query requires an index」で拒否される。エミュレータは索引を見ないので、テストでは分からない。
索引を足したら、本番に同じ問い合わせを投げて確かめる（作成には数分かかる）。
