...
| 移植元 | Loca | 状態 |
|---|---|---|
| `MarkerData.likeCount`（マーカーの項目） | 件数は `likeCounts/{markerId}` に置き、マーカー情報では開いたときに読む。公開データのマーカーにだけ `likes` を持つ（同期が入れる。購読では届かない。T92）。`youtube.likeCount` は YouTube の高評価数（ADR 0017）で、Loca のいいねとは別 | 済み（T29） |
| いいねした人の一覧 | 公開しない（本人と管理者だけが読める） | 済み（T29） |
| いいね数のランキング | 2026-09-26 に改め、ユーザーのタブで受け取ったいいねの順に投稿者を並べる（ADR 0031）。自分の投稿で受け取った件数も見られる | 済み（T92・T93） |
